...
export default function Freight_router({ quantity, cep, onFinalFreightValue }: FreightRouterProps) {
  ...
  const [finalFreightValue, setFinalFreightValue] = useState<number | null>(null);
  ...
  useEffect(() => {
    if (quantity > 0) {
      ...
      if (freightPrice && freightPrice.valor && freightPrice.peso) {
        if (quantity > 1) {
          const newFinalFreightValue = Math.ceil((freightPrice.peso * quantity) / 1000 + 1) * freightPrice.valor;
          setFinalFreightValue(newFinalFreightValue);
          onFinalFreightValue(newFinalFreightValue);
        }
        else {
          const newFinalFreightValue = Math.ceil((freightPrice.peso * quantity) / 1000) * freightPrice.valor;
          setFinalFreightValue(newFinalFreightValue);
          onFinalFreightValue(newFinalFreightValue);
  ...
  return (
    <div>
      ...
      <div className='flex flex-row'>
        <p className='font-bold mt-3 mb-3 flex flex-row'>
          Freight Value: R${' '}
          {finalFreightValue && (
            <p className='font-light'>{finalFreightValue.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
          )}
        </p>
      </div>
      <div className='flex flex-row'>
        ...
        {freightLocale && (
          <p className='flex flex-row items-center'>
            <Image src={v_location} alt='Locale' priority={true} />ㅤ {freightLocale.localidade}, {freightLocale.uf}.
          </p>
        )}
      </div>
    </div>
